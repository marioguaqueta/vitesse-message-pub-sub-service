locals {
  prefix = "vtss-${var.env}-${var.customer_id}"
  ...
  topics = {
    ...
    projection_updates   = { name = "${local.prefix}-projection-updates",   retention = "259200s" }
    maintenance_commands = { name = "${local.prefix}-maintenance-commands", retention = "86400s"  }
    media_commands       = { name = "${local.prefix}-media-commands",       retention = "259200s" }
  }
...
resource "google_pubsub_subscription" "crf_sender" {
  ...
  labels = merge(var.labels, { env = var.env, managed = "terraform", customer_id = var.customer_id })
}
